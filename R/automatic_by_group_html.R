...
set.caption(\"Table of most cited references for community ", deparse(as.numeric(current_community)),"\")
...
", sep = ""))
  ...
  if((length(suit_freq_tables$CR) / 3)%%1==0){
    final_core_html <- append(final_core_html, paste(table_type[current_table_type], big_body,sep = ""))
