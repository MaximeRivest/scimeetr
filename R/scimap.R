...
#' 
#' @param scimeetr_list A list of class scimeetr. Objects of class scimeetr are 
#'   return by with import_bib_file or a list of class scimeetr created from the
#'   function scimap.
#' @param coupling_by A character vector of length one. Equal to either: 
#'   'bic'/'bibliographic coupling', 'kec'/'keyword coupling', 'tic'/'title 
#'   coupling', 'abc'/'abstract coupling', 'joc'/'journal coupling', 
...
#'   community, with all their associated graph object, community object and 
#'   biblio_df object.
#' @export
#' @import dplyr purrr
...
scimap <- function(scimeetr_list, 
  ...
  for(i in 1:length(scimeetr_list)){
    ...
    if(sum(stringr::str_detect(names(i_lsci), 'graph')) >= 1){
      ...
      graph_object = coupling(i_lsci$dfsci, coupling_by, kw, ti, ab)
